...
class ItemSimilarity(Similarity):
    ...
    to 1.0, with 1.0 representing perfect similarity.
    '''
    def __init__(self, model, distance, numBest=None,topicDistribution=None, movieIndex=None,distributionCache=None):
        Similarity.__init__(self, model, distance, numBest)
        self.topicDistribution=topicDistribution
        self.movieIndex=movieIndex
        self.distributionCache=distributionCache

    def getSimilarity(self, vec1, vec2):
        if self.distributionCache !=None:
            return self.distance(vec1, vec2,divergence=self.distributionCache)
        if self.topicDistribution ==None:
            # Evaluate the similarity between the two users vectors.
            #print 'hi'
            ...
            #print self.distance(vec1, vec2, movieIndex)
            return self.distance(vec1, vec2, self.topicDistribution,self.movieIndex)
